	...
	var Drawing = useSVG ? svgDrawer : !_isSupportCanvas() ? (function () {
		...
		Drawing.prototype.draw = function (oQRCode) {
			...
			if(_htOption.title){
				...
				aHTML.push('<div style="width:100%;color:'+c+';font:'+f+';background:'+_htOption.titleBgColor+'">'+_htOption.title+'</div>');
			}
			if(_htOption.subTitle){
				aHTML.push('<div style="width:100%;margin-top:'+(_htOption.subTitleTop-_htOption.titleTop)+'px"></div>');
				aHTML.push('<div style="width:100%;color:'+_htOption.subTitleColor+'; font:'+_htOption.subTitleFont+'">'+_htOption.subTitle+'</div>');
			}
			aHTML.push('</td></tr>')
			for (var row = 0; row < nCount; row++) {
				aHTML.push('<tr>');
			...
			if(_htOption.logo){
				...
				aHTML.push('<tr height="0" ><td align="center" style="border:0;border-collapse:collapse;margin:0;padding:0" colspan="'+nCount+'">');
				...
				aHTML.push('<div style="height:0"><img  src="'+_htOption.logo+'" width="'+imgW+'" height="'+imgH+'"  style="position:relative; top:-'+(_htOption.height)/2+'px; z-index:1;"/></div>')
				aHTML.push("</td></tr>");
			}
...
//				}
	...
	QRCode = function (el, vOption) {
		this._htOption = {
			...
			title: "", // 标题
			titleFont:"bold 16px Arial", // 标题字体
			titleColor:"#000", // 标题颜色
			titleBgColor:"#fff", // 标题背景
			titleHeight: 50, // 标题高度，包含 subTitle
			titleTop: 30, // 标题绘制 y 坐标
			
			subTitle:"", // 副标题内容
			subTitleFont:"14px Arial", // 副标题字体
			subTitleColor:"gray", // 副标题颜色
			subTitleTop: 50 // 副标题绘制 y 坐标
		};
